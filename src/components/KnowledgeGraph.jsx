...
export default function KnowledgeGraph({ onNavigate }) {
  ...
  const [completed, setCompleted] = useState(() => {
    try {
      const saved = JSON.parse(window.localStorage.getItem("zhichuan-knowledge-progress") || "[]");
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem("zhichuan-knowledge-progress", JSON.stringify([...completed]));
    } catch {
      // localStorage 不可用时仅保留当前会话进度。
    }
  }, [completed]);
