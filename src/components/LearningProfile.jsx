    ...
    <div className="space-y-8">
      ...
      <section className="rounded-2xl border border-cyan/20 bg-panel/80 p-6 shadow-glow">
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-2">
          ...
          <ReportCard title="AI成长导师" tone="mint" items={growthMentorAdvice(record, insight)} button="进入 AI 工作流" onClick={() => onNavigate("workflow")} />
...
function growthMentorAdvice(record, insight) { const weakest = insight.abilities.slice().sort((a, b) => a.stars - b.stars)[0]; const advice = [`当前优先提升：${weakest.name}。${insight.next[0]}`]; if (!record.chapters.length) advice.push("先完成一个知识图谱章节，建立课程概念与数据方法的连接。"); else if (!record.labUsage) advice.push("进入智能分析实验室完成一次小样本试分析，把学习记录转化为数据实践。"); else if (!record.project) advice.push("将已有研究想法整理为学生项目卡片，形成可展示的课程成果。"); else advice.push("项目成果已形成，建议使用 AI项目评价智能体进行修改并进入下一轮能力复盘。"); return advice; }
